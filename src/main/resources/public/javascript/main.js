...
let fileTemplate = Handlebars.compile($("#fileTemplate").html());

// 첨부파일 drag & drop 영역 선택자
let fileDropDiv = $(".fileDrop");
...
function deleteFileWrtPage(that) {
	let url = "/article/file/delete";
	deleteFile(url, that);
}

// 파일 삭제(수정페이지) : 서버에 저장된 첨부파일과 데이터베이스에 저장된 첨부파일 정보 삭제 처리
function deleteFileModPage(that, articleNum) {
	let url = "/article/file/delete/" + articleNum;
	deleteFile(url, that);
}
...
// 파일 목록 : 게시글 조회, 수정페이지
function getFiles(articleNum) {
	$.getJSON("/article/file/list/" + articleNum, function (list) {
		if (list.length === 0) {
...
function getFileInfo(fullName) {
	...
	if (checkImageType(fullName)) {
		...
		let originalImg = fullName.substr(0, 12) + fullName.substr(14);
		// 원본 이미지 요청 링크
...
function checkImageType(fullName) {
	let pattern = /jpg$|gif$|png$|jpge$/i;
